...
function new_link() {
	...
	var delLink =
		...
		'<td class="text-end">' +
		"<div>" +
		'<input type="text" class="form-control bg-light border-0 product-line-price" id="ProductLocation-' + count + '" value="$0.00" placeholder="$0.00" />' +
		"</div>" +
		"</td>" +
...
if ((viewobj != '') && (options == "edit-invoice")) {
	...
	setTimeout(() => {
		Array.from(paroducts_list).forEach(function (element) {
			...
			document.getElementById("ProductLocation-" + counter_1).value = "$" + ((element.rates) * (element.quantity));
			counter_1++;
		});
	}, 300);
...
document.addEventListener("DOMContentLoaded", function () {
	...
	formEvent.addEventListener("submit", function (event) {
		...
		Array.from(products).forEach(element => {
			...
			var product_price = (element.querySelector("#ProductLocation-"+count).value).split("$");;
